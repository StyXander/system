...
function renderRiskCard() {
  ...
  // gap 不大于 0 只表示当前 R1 方向未出现，不代表没有其他风险。
  if (gap > 0) {
    ...
    $("#risk-observation").textContent = `${currentYear}，${company}应收账款增速为${percent(arGrowth)}，营业收入增速为${percent(revenueGrowth)}。按当前两个指标的方向，未出现应收账款增速高于收入增速的现象；这不代表公司不存在其他风险。`;
