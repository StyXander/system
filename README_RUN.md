...
1. 首先看到“证据地平线”项目首页；点击“进入评委演示”一次直达工作台，工作台默认选中标准股份，也可从 3 个精选案例或“查看全部 15 个案例”切换公司；“方法与边界”保留为次级阅读入口；
...
4. 结果区先展示“30 秒证据挑战”：核对同一 `run_id` 下的主张、`evidence_id`、关键数字、反证/复核状态和人工下一步，再一次点击打开原文页；下方继续展示结构化待核查事项、证据追踪轴、语义表格和“系统替你完成什么”程序映射，可下载 JSON/CSV、打印或保存 PDF，也可打开全部原文证据和三 Agent 过程抽屉；
...
竞赛演示页不暴露上传、字段确认、缓存管理、补充资料和正式复核写入入口。顶部“技术说明”和“现场样例”已收进“次级入口”，不参与评委主线；只有本地启动器设置 `AUDITTRACE_ONSITE_LIVE_SAMPLE=true` 时才允许创建非内置巨潮任务，云端共享部署默认关闭。
...
2026-09-03 当前口径（取代下一段 2026-09-02 记录；下一段仅保留作历史溯源）：源码仓库全量 **373 passed, 1 warning / 122.51s**（37 文件，日志 `tmp/ai2-final-repo-pytest-20260903-v2.txt`；整改前基线 368 passed 见 `tmp/baseline-repo-pytest-20260903-prechange.txt`）。保留意见：历史同一命令曾出现过 1 项间歇性 `sensitive_data_blocked` 失败（`test_competition_demo_plan.py:798`，案例 `CNINFO_601988_T0_20260330`），本次全量复跑通过但根因尚未完成独立复现，因此不得对外写作“稳定全绿”，见 `tmp/incident-20260903-间歇性隐私闸门阻断.md`。评委代码复现包最后一次全新解包实测 **346 passed, 19 skipped, 0 failed**（18 项 requires_full_corpus、1 项内部签字边界；新 Python 3.12 清洁环境复验为 2 warnings，最终证据见 `交付包/交付清单与扫描报告.txt`）；此前一次后续 validate-only 曾在 `CNINFO_600000_T0_20260330` 观察到 `sensitive_data_blocked`，该案例单测和再次全新解包全量均通过，观察记录见 `tmp/incident-20260903-评委包第二次间歇性隐私阻断.md`，根因仍未知，不能把评委包写成稳定全绿。新包已由打包脚本带验证器重建。中文注释 **2416/23991 = 10.07%**。本轮已把 B3 真实性判据从测试内部搬进生产模块 `backend/app/release_gate.py`，并给打包脚本加了“登记数字与实测不一致即拒绝出包”的硬校验；API 现在也调用同一门禁。尚未通过的门：当前工作区仍 dirty，不能把当前改动称作干净 git 发布快照；历史“19 项失败”名单与根因仍为 UNKNOWN，R1 签字、生产新鲜 B3、人工评分和最终发布批准均 pending。复现步骤见 `docs/superpowers/plans/2026-09-03-AI-2最终收口执行计划.md`。
