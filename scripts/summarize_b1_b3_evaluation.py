...
def main() -> int:
    ...
    lines += [
        ...
        "## 低于 80% 告警",
        "",
        "质量窗口只按当前配置模型（以 `.env` 的 `DEEPSEEK_MODEL` 与 `PROJECT_STATUS.json` 登记一致）最近 10 次真实外部三 Agent 完整运行统计；低于 80% 阈值时如实标注 `below_threshold`、`alert=true`，新通道样本不足时标注未测量。历史 qwen3.5-plus 的 7/10=70.0% 只作冻结证据。B2 与 B3 口径不同，不合并为官方成功率；不自动更换模型。",
        "",
