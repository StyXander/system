...
def create_cninfo_pipeline(
    ...
    background_tasks.add_task(_execute_cninfo_task, task["task_id"], task["request"], http_request)
    return _with_ai_notice(
        {
...
def get_cninfo_pipeline(task_id: str) -> dict[str, Any]:
    """读取巨潮任务进度和每一步证据。"""

    task = load_task(WORKSPACE_ROOT, task_id)
    if task is None:
        raise HTTPException(status_code=404, detail="未找到巨潮导入任务。")
    return _with_ai_notice(task)


@app.get("/api/pipelines/{task_id}/result")
def get_cninfo_pipeline_result(task_id: str) -> dict[str, Any]:
...
def confirm_cninfo_company(
    ...
    try:
        ...
        raise HTTPException(status_code=409, detail=str(error)) from error
    background_tasks.add_task(_execute_cninfo_task, queued["task_id"], queued["request"], http_request)
